            ...
            for msg_id in Conversations.objects.get(conversation_id=int(conv_id)).conversation_history.split(","):
                if msg_id != '':
                    ...
                    if time_dif < 3 and request.COOKIES.get('id', None) != msg.user_id:
                        cur_time = datetime_from_utc_to_local(msg.message_time) .strftime('%b. %d, %Y, %-I:%M %p').replace('AM', 'a.m.').replace('PM', 'p.m')
    ...
    if not logged_in:
        ...
        if request.method == 'POST':
            ...
            if form.is_valid():
                ...
                discussion.save()
                profile_ids = [val.user_id for val in UserProfiles.objects.all() if str(user_id) in val.user_following.split(",")]
                username = UserProfiles.objects.get(user_id=user_id).username
...
def clear_all_notification(request):
    logged_in, user_id = verify_request(request)
    if logged_in:
        NotificationMessages.objects.get(notification_user_id=user_id).delete()
    return redirect('/')
